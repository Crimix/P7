...
\begin{requirement}{Web-Application Frontend}
The application should make use of a frontend developes as a web application.
This application should be hostable on a server such that it can be presented as
a webpage.
\end{requirement}

\begin{requirement}{Server Backend}
The logic of the developed software should be hosted on a server, which recieves
requests from the frontend, and returns analyzed data and the determiend
conclusions.
\end{requirement}
...
The server backend must be capable of handling an undefined number of concurrent
user requests, as if it acted as the backend of an actual webpage. As such, the
server must queue requests, and make sure to handle them all.
\end{requirement}
...
calls to the Twitter API, the frontend should make use of the official
...
\begin{requirement}{API Communication}
The developed software must make use of APIs in order to communicate between
eachother. These APIs must use verification such that only approved requests are
processed. These APIs should be used for the database, and the server backend.
\end{requirement}
