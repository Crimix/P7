...
Many corrections on the report, some missing introductions and conclusions.
From the meeting we found out that we wanted a \ac{GUI} web application in
Laravel \fix{ToDo}{Ref til Laravel} which should connect to a queue server in
C\# through a \ac{REST} \ac{API}. Then the queue should start call a modified
version of our workers: program fetching tweets from Twitter. The \ac{GUI} and
workers communicate with a databse through another \ac{REST} \ac{API}.
We split all members into different parts and progressed on all, investigating
how and what we should do with them. We quickly found that the database
\ac{REST} \ac{API} should be made in a separate Laravel project.

\subsection*{Evaluation}
During the presentation Monday we recieved a lot of feedback on everything, and
some keywords or methods extreemly useful to us that we had not considered. A
presentation meeting, where we show our solution and talk about our ideas and
methods, is a very good idea and definetly strengthen the project. New projects
for the solution, found through brainstorming, we had not considered before,
was added to our complete solution.

\subsection*{Next Week}
During the next week we will continiue development on the parts, we have a lot
to do after this meeting. And we will make some considerations on which type of
user we want, university, highschool or other?
The webcrawler chapter should also be expanded with more info on the \ac{API}
and perhaps some information on why we describe the crawler, since we wont
really use it.
