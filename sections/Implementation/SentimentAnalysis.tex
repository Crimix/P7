...
\fix{The models that are of interest to use are the Bag-of-Words model, the Naive
Bayes Classifier, the Support Vector Machine and the Neural Network.}{add
...
%I did the thing?
The model which we use for sentiment analysis is a simple unigram language
model called a ``bag-of-words'' model. This approach determines the sentiment
of a text by counting the occurrences of words with either a negative of
positive sentiment. These words can have a different sentiment score, that is,
one word may be percieved as conveying a more intense sentiment than others.
In our model we have chosen to use a simplified approach, where each word has a
value of either -1 or 1 for negative and positive sentiment respectively. The
reson for choosing this approach is, that we have chosen to use existing
freely available list of sentimental words to use in our analysis, and these
list are not using variable values for words.