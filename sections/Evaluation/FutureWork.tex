...
\section*{Queue API Authorization}\label{disc:auth}
...
The Naive Bayes and Bag-Of-Words models are simplistic models, and on other
data sets are outperformed by more advanced models, such as the Support Vector
Machine and Neural Network. In the future it would be interesting to see how
these models measure up to the ones we currently use.
...
the models to be. This would be especially helpful in judging how accurate our
current models are.
...
authorization information. This would be a major problem when releasing the
...
would be the case is if the user tries to get the filter bubble of a private
